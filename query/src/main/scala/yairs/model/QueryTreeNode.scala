 ...
 * Time: 10:36 AM
 */
class QueryTreeNode(queryOperator: String, subQuery: String) extends Logging {
  ...
  val subStringParts = PrefixBooleanQueryParser.split(queryString)
  ...
  def dump(layer: Int) {
    ...
    if (!isLeaf) println(operator) else println(term + " : ["+ field+"]")
  }
